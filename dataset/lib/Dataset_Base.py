"""
 @Author  : Shuai Liao
"""
from dataset.lib.imagedata_lmdb import ImageData_lmdb
from dataset.lib.common import add_path, env, rdict, cv2_wait, cv2_putText, is_py3
import os
import sys
import numpy as np
...
def get_anno(db_path, category_target):  # target=''
    ...
    viewID2quat = pickle.load(
        open(os.path.join(db_path, 'viewID2quat.pkl'), 'rb'), encoding='latin1')
    viewID2euler = pickle.load(
        open(os.path.join(db_path, 'viewID2euler.pkl'), 'rb'), encoding='latin1')
    keys = np.array(list(viewID2quat.keys()))
    idx_target = []
    from lib.db_type import img_view_anno
    ...
    for i, (key, quat) in enumerate(viewID2quat.items()):
        ...
        rc.cad_id = cad_id
        # q and -q give the same rotation matrix.
        rc.so3.quaternion = quat if quat[0] > 0 else -quat
...
class Dataset_Base(Dataset):
    ...
    def __init__(self, base_dir, category, collection='train', net_arch='alexnet', sampling=1.0):
        ...
        self.db_path = os.path.join(
            base_dir, 'ModelNet10-SO3', self.collection2dbname[collection])
        ...
        if sampling < 1.0:
            ...
            sample_inds = np.random.choice(_inds, size=int(
                len(_inds) * sampling), replace=False)
            sample_inds.sort()
            self.keys, self.recs = [self.keys[x]
                                    for x in sample_inds], self.recs[sample_inds]
    ...
    def __getitem__(self, idx):
        ...
        """
        print(
            'This is an interface method, and you need to implement it in inherited class.')
        raise NotImplementedError
