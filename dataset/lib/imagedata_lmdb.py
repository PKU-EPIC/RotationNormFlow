...
import numpy as np
import cv2
from dataset.lib.yaml_netconf import load_yaml, dump_yaml
from dataset.lib.common import add_path, env, Timer, Checkpoint, cv2_wait, is_py3
import os
import sys
...
class Handle_Rawebp(Data_Handler):
    ...
    def pack_from_npyarr(self, imgID, npyarr):
        retval, buf = cv2.imencode(
            '.webp', npyarr, [cv2.IMWRITE_WEBP_QUALITY, self.QUALITY])
        assert retval == True
        rawbytes = buf.tostring()
        #
        key = imgID.encode('utf8')  # ascii
...
class Handle_Rawpng(Data_Handler):
    ...
    def __init__(self, always_load_color=True, remap=None):
        ...
        if remap is not None:
            # if remap is need, always load unchanged.
            self.cv2_IMREAD_FLAG = cv2.IMREAD_UNCHANGED
            #
            if hasattr(remap['min'], '__len__'):
                # channel of pixels (Can be 1,2,3,4)
                assert len(remap['min']) == len(remap['max'])
                self.c = len(remap['min'])
            else:
                self.c = 0
            assert remap['png_dtype'] in [
                'uint8', 'uint16'], remap['png_dtype']
            self.png_PXL_MAX = dict(uint8=255, uint16=65535)[
                remap['png_dtype']]
            remap['npy_dtype'] = eval(
                'np.' + remap['npy_dtype'])  # make it as np.dtype
            remap['png_dtype'] = eval(
                'np.' + remap['png_dtype'])  # make it as np.dtype
    ...
    def pack_from_npyarr(self, imgID, npyarr):
        if self.remap is not None:
            ...
            # map to [0.0,1.0]
            npyarr = (npyarr - self.remap['min']) / \
                (self.remap['max'] - self.remap['min'])
    ...
    def unpack(self, data_bytes):
        ...
        if self.remap is not None:
            if self.c > 0:
                # map to [0.0,1.0]
                img = (img[:, :, :self.c] /
                       self.png_PXL_MAX).astype(self.remap['npy_dtype'])
            else:  # single channel image
                # map to [0.0,1.0]
                img = (img / self.png_PXL_MAX).astype(self.remap['npy_dtype'])
...
class ImageData_lmdb:
    def __init__(self, db_path, mode='r', map_N=30000, max_readers=256, always_load_color=True, silent=False,
        ...
        """
        self.db_path = os.path.abspath(db_path.rstrip(
            "/"))  # remove last '/' if there's any.
        # assert os.path.exists(self.db_path)
        self.mode = mode
        if self.mode == 'w':
            # overwrite if overwrite and os.path.exists(self.db_path):
            os.system('rm -rf %s' % self.db_path)
        ...
        if self.mode == 'r':
            ...
            self.lmdb_env = lmdb.open(
                self.db_path, map_size=self.map_size, max_readers=max_readers)  # lock=True

        if self.db_path.endswith('.Rawjpg.lmdb'):
            if not silent:
                print("[Using] Handle_Rawjpg")
            ...
            print(
                'Unrecognized imagedata_lmdb extension:\n[db_path] %s' % self.db_path)
            raise NotImplementedError
        ...
        if os.path.exists(os.path.join(self.db_path, 'imgId2dbId.pkl')):
            ...
            self.imgId2dbId = pickle.load(
                open(os.path.join(self.db_path, 'imgId2dbId.pkl'), 'rb'))  # OrderedDict
        else:
            ...
            if not os.path.exists(key_cache_file):
                ...
                with self.lmdb_env.begin() as txn:
                    keys = [key.decode('utf-8')
                            for key, _ in txn.cursor()]  # very slow!!!
    ...
    def __setitem__(self, key, npyarr):
        ''' key is usually imgID, normally we need key as bytes.'''
        assert self.mode in [
            'w', 'a+'], "Not open in write mode: %s " % self.mode
    ...
    def compute_mean_std(self, sample_k=None):
        ...
        pxl_mean = np.array([2.19471788e-05, 2.19471788e-05,
                            2.19471788e-05], np.float32) * C1
        ...
        for i, key in enumerate(keys):
            ...
            if len(data.shape) == 3:
                ...
                (h, w), c = data.shape, 1
            sum_var += np.sum(((data.reshape(-1, c) - pxl_mean)
                              ** 2) / C2, axis=0)
            cnt += h * w
            if i % 1000 == 0:
        ...
        # [2.19471788e-05 2.19471788e-05 2.19471788e-05]
...
def test_lmdb():
    ...
    lmdb_env = lmdb.open(
        env.Home + '/working/cvpr17pose/dataset/PASCAL3D/ImageData.Max500.Rawjpg.lmdb', map_size=10000)
    handle = Handle_Rawjpg()
...
def test_img_lmdb():
    imgdb = ImageData_lmdb(
        env.Home + '/working/cvpr17pose/dataset/PASCAL3D/ImageData.Max500.Rawjpg.lmdb')
...
def show_lmdb():
    ...
    parser.add_argument('-f', '--format', default='5x6',
                        type=str, help='rows and cols of display.')
    parser.add_argument('-s', '--side', default=150, type=int,
                        help='max side len of each image.')
    parser.add_argument('-r', '--random', action="store_true",
                        default=False, help='random show entries in DB.')
