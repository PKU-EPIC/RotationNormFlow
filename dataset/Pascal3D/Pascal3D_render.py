...
import numpy
import glob
from dataset.Pascal3D import Pascal3D
...
class Pascal3DRendered(Dataset):
    def __init__(self, path, image_size=224, category=None):
        path = os.path.join(path, DATASET_FOLDER_NAME)
        self.path = os.path.join(path, DATASET_FOLDER)
        if category is not None:
            cate_id = pascal_class_to_id[category]
        else:
            cate_id = '*'
        all_files = glob.glob(os.path.join(
            self.path, f'{cate_id}/*/*.jpeg')) + glob.glob(os.path.join(self.path, f'{cate_id}/*/*.png'))
    ...
    def __getitem__(self, idx):
        ...
        img_full = np.array(img_full.getdata()).reshape(
            img_full.size[1], img_full.size[0], 3).astype(np.float64) / 255
        ...
        principal_point = np.array(
            [img_full.shape[1] / 2, img_full.shape[0] / 2], dtype=np.float64)
        ...
        desired_up = np.array([3.0, 0.0, 0.0]) + \
        ...
        warped_image = skimage.transform.warp(img_full, transform, output_shape=(
            desired_imagesize, desired_imagesize), mode='constant', cval=0.0)

        class_enum = id_to_pascal_class[synsetID]

        return {'img': warped_image.transpose(2, 0, 1).astype(np.float32), 'rot_mat': extrinsic_after[:3, :3].astype(np.float32), "cate": int(class_enum)-1, "easy": 0}
        return warped_image.transpose(2, 0, 1).astype(np.float32), extrinsic_after.astype(np.float32), int(class_enum), False, intrinsic_new.astype(
            np.float32), 0
...
def aet_to_R(a, e, t):
    ...
    R2th = np.array([[np.cos(t), -np.sin(t), 0],
