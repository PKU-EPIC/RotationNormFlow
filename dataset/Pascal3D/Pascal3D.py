...
def mat_data_to_dict_data(mat_data, folder):
    ...
    ret[DICT_FILENAME] = os.path.join(
        folder, get_mat_element(record['filename']))
    return ret
        ...
        for filename in os.listdir(folder_unsplit_dir):
            ...
            for obj in data[DICT_OBJECT_LIST]:
                ...
                save_path = os.path.join(
                    directory_split, '{}.json'.format(count))
...
def get_camera_matrix(camera, principal_point, angle, image_size, distance):
    ...
    R2d = np.array([[np.cos(th), -np.sin(th), 0],
    ...
    # + np.array([image_size[1]/2.0, image_size[0]/2.0]) # image size is hxw
    pp = principal_point
...
def get_minimum_covering_sphere(points):
    ...
    def sphere_welzl(points, included_points, num_included_points):
        if len(points) == 0 or num_included_points == 3:
            ...
            p = points[0]
            rem = points[1:]
            cand_mid, cand_rad = sphere_welzl(
                rem, included_points, num_included_points)
...
def show_crop_bounding_box(img, bbox):
    ...
    if maxy < shape[0]:
        ...
        end_y = shape[0] - miny
        full_end_y = shape[0]
    im_show[offset_y:end_y, offset_x:end_x] = img[full_start_y:full_end_y +
                                                  1, full_start_x:full_end_x + 1] / 255
    plt.imshow(im_show)
    plt.show()
...
class Pascal3D():
    def __init__(self, dataset_location=None, image_size=224, train_all=False, use_warp=True, voc_train=False, source=None, category=None):
        ...
        if not os.path.exists(self.json_annotation_path()):
            ...
            split_json_annotations(
                json_annotation_path_tmp, self.json_annotation_path())
    ...
    def get_split(self, validation_split_size=0.3):
        ...
        for split_name in split_names:
            split = []
            for cls in classes:
                filepath = os.path.join(dataset_split_path, str(
                    cls) + '_imagenet_' + split_name + '.txt')
                with open(filepath, 'r') as f:
                    while True:
                        ...
                        img_path = os.path.join(
                            str(cls) + '_imagenet', l + '.JPEG')
                        split.append(img_path)
            splits.append(split)
        test_split = splits[1]
        rest = splits[0]
        rest = sorted(rest)
        val_idx = (np.arange(len(rest) * validation_split_size) /
                   validation_split_size).astype(np.int64)
        ...
        elif self.source == 'both_both':
            source_train_split = imagenet_train_split + pascal_train_split
            source_test_split = imagenet_test_split + pascal_test_split
        else:
            raise ValueError

        ret = get_dataset_indices_from_imagefilenames(
            [source_train_split, source_val_split, source_test_split], self.json_annotation_path())
    ...
    def get_cad(self, class_idx, cad_idx):
        ...
        cad_file = os.path.join(self.location, 'CAD',
                                class_str, '{0:02d}.off'.format(cad_idx))
...
class Pascal3DSubsetWarp(Dataset):
    ...
    def __getitem__(self, idx):
        sample_file = os.path.join(
            self.json_directory, "{}.json".format(self.files[idx]))
        sample_data = None
        with open(sample_file, 'r') as f:
            sample_data = json.load(f)
        im_path = os.path.join(self.image_directory,
                               sample_data[DICT_FILENAME])
        with open(im_path, 'rb') as f:
            img_PIL = Image.open(f)
            img_PIL.convert('RGB')
            data = img_PIL.getdata()
            if isinstance(data[0], int) or len(data[0]) == img_PIL.size[1] * img_PIL.size[0]:
                img_full = np.array(data).reshape(img_PIL.size[1], img_PIL.size[0]).reshape(
                    img_PIL.size[1], img_PIL.size[0], 1).repeat(3, 2)
            else:
                img_full = np.array(data).reshape(
                    img_PIL.size[1], img_PIL.size[0], 3)
        ...
        if self.augmentation:
            ...
            desired_up = np.random.normal(
                0, 0.4, size=(3)) + np.array([3.0, 0.0, 0.0])
            desired_up[2] = 0
            desired_up /= np.linalg.norm(desired_up)
            # # jitter bounding box
            bbox_w = bbox[2] - bbox[0]
            bbox_h = bbox[3] - bbox[1]
            bbox[0::2] += np.random.uniform(-bbox_w *
                                            0.1, bbox_w * 0.1, size=(2))
            bbox[1::2] += np.random.uniform(-bbox_h *
                                            0.1, bbox_h * 0.1, size=(2))
        else:
            desired_up = np.array([1.0, 0.0, 0.0])

        intrinsic, extrinsic = get_camera_parameters(
        ...
        extrinsic_desired_change, intrinsic_new = get_desired_camera(
            desired_imagesize, back_proj_bbx, desired_up)
        extrinsic_after = np.matmul(extrinsic_desired_change, extrinsic)

        P = np.matmul(np.matmul(
            intrinsic_new, extrinsic_desired_change[:3, :3]), np.linalg.inv(intrinsic))
        P /= P[2, 2]
        Pinv = np.linalg.inv(P)
        transform = skimage.transform.ProjectiveTransform(Pinv)
        im = img_full.astype(np.float64) / 255
        warped_image = skimage.transform.warp(im, transform, output_shape=(
            desired_imagesize, desired_imagesize), mode='constant', cval=0.0)
        ...
        return {'img': warped_image.transpose(2, 0, 1).astype(np.float32), 'rot_mat': extrinsic_after[:3, :3].astype(np.float32), "cate": int(class_idx)-1, 'easy': 1-int(hard)}
        return warped_image.transpose(2, 0, 1).astype(np.float32), extrinsic_after.astype(np.float32), int(class_idx), hard, intrinsic_new.astype(
            np.float32), int(cad_idx)
...
class Pascal3DSubsetCrop(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # plt.show()
        im = np.array(Image.fromarray(im.astype(np.uint8)).resize(
            (self.image_out_size, self.image_out_size), Image.BILINEAR))
        im = im.astype(np.float32) / 255
        intrinsic = np.array([[self.image_out_size * 8, 0, self.image_out_size / 2],
                              [0, self.image_out_size * 8,
                                  self.image_out_size / 2],
                              [0, 0, 1]])
    ...
    for i in range(len(splits)):
        for j in range(len(splits)):
            if i != j:
                ...
                if len(intersect) != 0:
                    print('split {} and {} have filename intersection {}'.format(
                        i, j, intersect))
    ...
    for i in range(len(ret)):
        for j in range(len(ret)):
            if i != j:
                ...
                if len(intersect) != 0:
                    print('split {} and {} have index intersection {}'.format(
                        i, j, intersect))
