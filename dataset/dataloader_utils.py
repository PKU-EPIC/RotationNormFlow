...
class MixDataset(torch.utils.data.Dataset):
    def __init__(self, datasets):
        ...
        for dataset in datasets:
            self.lens.append(dataset.__len__())
        total = 0
        for length in self.lens:
            total += length
        self.total = total
        self.shuffle_list = list(range(total))
    ...
    def __getitem__(self, raw_idx):
        index = self.shuffle_list[raw_idx % self.total]
        total = 0
        for i in range(len(self.datasets)):
            length = self.lens[i]
            if total+length > index:
                return self.datasets[i].__getitem__(index-total)
            total += length
